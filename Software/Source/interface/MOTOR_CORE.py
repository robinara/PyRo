...
class DC:
	...
	def setMotorPower(self,duty,direction):
		try:
			...
			print inst 
		if abs(direction)==1:
			if self.__MOTOR_CLASS=="VNH5019":	
				self.__sendCommand__(self.__pin_PWM,duty)
				if direction==1:
					self.__sendCommand__(self.__pin_INA,1)
					self.__sendCommand__(self.__pin_INB,0)
				if direction ==(-1):
					self.__sendCommand__(self.__pin_INA,0)
					self.__sendCommand__(self.__pin_INB,1)
				if self.__MOTOR_CLASS=="24v23":
					self.__sendCommand__(self.__pin_PWMH,duty)
					self.__sendCommand__(self.__pin_PWML,duty)
				if direction==1:
					self.__sendCommand__(self.__pin_DIR,1)
				if direction ==(-1):
					self.__sendCommand__(self.__pin_DIR,0)
		return

	##Set the pulse width for the servo.
	def setServoPower(self,duty):
		self.__sendCommand__(self.pin_PWM,duty)
		return
